...
<strong>本番のサーバーに向けてテストを走らせない。</strong>テスト用のサーバーを別に立てて、そちらへ繋ぐ。

```batch
N:\2026\ai-chat-lite\tools\40_test\start-test-server.cmd
```

本番とは別のポート・別の DB で立つ。**本番は止めない**（ポートが違えば共存できる）。

ポートは空きを探して決まるため、固定ではない。決まった値は `tmp/test-server.json` に書かれるので、テスト側はこれを読む。
...
終わったら止める。**DB も一緒に捨てられる。**

```batch
N:\2026\ai-chat-lite\tools\40_test\stop-test-server.cmd
```
