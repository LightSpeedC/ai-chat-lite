...
<strong>サーバーに繋ぐコマンドは、何かする前に繋ぎ先を stderr に出す。</strong>先頭の 1 行が `本番（:8787）` なら本番、`テスト（:8765）` ならテスト用のサーバーである。**`waiters` はサーバーに繋がないので出さない。**

> [!IMPORTANT]
> <strong>本番は `test-` で始まる ID と `sandbox-` で始まるルームを受け付けない。</strong>渡すと 400 で断られる。

> [!CAUTION]
> <strong>確認や試しは本番でやらない。</strong>テスト用のサーバーを立てて、そちらへ繋ぐ。
