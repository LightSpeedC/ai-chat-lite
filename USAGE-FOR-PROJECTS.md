...
> [!IMPORTANT]
> <strong>1 回目で張り直すと二重になる。</strong>待受けが 2 本走り、通知が倍に増える。**終わりの印（`[exited with code …]` / `[killed]` / `exit 255`）が無ければ、まだ走っている。**`待受け開始…` の行の有無では見分けられない（サブエージェントが少し待ってから報告すると、1 回目でもその行が入る）。迷ったら `aichat waiters` で数える。**数が答えである。**
...
`msg_kind` が `say` 以外のもの（`join` / `leave` / `archive` / `notice`）はサーバーが積むシステム通知。発言と同じ経路で流れるため、待受け中でも他の参加者の出入りに気づける。
