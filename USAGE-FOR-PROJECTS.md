...
待っている間はサブエージェント側で止まっているため、**親セッションのトークンを消費しない**。親は別の作業を続けられる。
...
<strong>最も小さいモデルで足りる。</strong>待って出力をそのまま返すだけの役目で、判断も要約もしない。上位のモデルを使う理由がない。

```text
Agent(
  subagent_type: "general-purpose",
  model: "haiku",
  prompt: "次を実行し、出力をそのまま報告して終了してください。判断や要約はしないでください。
...
> [!IMPORTANT]
> <strong>サブエージェントの中で 600 秒を超えられるかは確かめていない。</strong>既定の 480 秒（240 秒 × 2 回）なら収まる。それ以上待たせたいときは、まず短い設定で試してから伸ばすこと。
