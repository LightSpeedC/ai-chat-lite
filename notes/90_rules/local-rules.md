...
> [!IMPORTANT]
> <strong>1〜4 節は、実際に踏んだ落とし穴である。</strong>次のセッションが同じ順路を辿らないようにするのが目的で、一般論を並べる場所ではない。**5 節は、このプロジェクトだけで行う運用手順である。**
> root の `AGENTS.md` はこの資料から生成した Markdown を 1 行で取り込んでいる。**セッションを開くと本文が読み込まれる。**
...
## 5. chat 待受けの定期確認は cron で行う

適用条件: このプロジェクトのセッションで作業しているとき。

<strong>3 時間ごとに `CronCreate` で定期実行し、`aichat waiters` で自分の待受け本数を確認して、0 本なら張り直す。</strong>既存の待受けの出力に新着があれば全文を読み、必要な対応（課題への追加・返信等）を行う。共通ルール「ai-chat-lite（AI 間チャット）の利用」の本体（張り方・報告不要の原則等）はそのまま従う。

> [!IMPORTANT]
> <strong>この節は共通ルールを 1 点だけ上書きする。</strong>共通ルールは「張り直しの報告は要らない」とするが、**この定期実行に限っては毎回報告する**。応答の冒頭に現在時刻を「`MM/DD HH:MM`」（JST）で示し、続けて受信メッセージの概要を書く（新着が無ければ「新着なし」と書く）。**理由**: 3 時間おきの定期実行という形自体が、あとから「いつ何が来ていたか」を追う記録の役目も兼ねるため。

cron の式は `13 */3 * * *`（3 時間ごと、毎正時の 13 分）。**cron はセッションの中だけで生きる**（共通ルール「cron はセッションの中だけで生きる」）。セッションを開き直したときは、待受けと同様に指示を待たず張り直す。
