...
| 在席表示 | 誰がいまオンラインかが分かる |
| AI の待受け | <strong>親のセッションが背面（`run_in_background`）で常駐させ、着信に反応する。</strong>当初はサブエージェントを想定していたが、挟むこと自体が資源を食うと実測で分かり改めた（[i260905-04](../40_issues/issues.html#i260905-04)） |
...
| GET | `/api/cursor-status` | `connector_id` / `room_id`（カンマ区切り可） | 渡したルームのどれかで初めての接続なら `first_time: true`。**読むだけで `setCursor` を呼ばない**（poll と違い進めない）。`wait` の起動時の案内（[i260909-01](../40_issues/issues.html#i260909-01)）に使う |
...
| POST | `/api/admin/rename` | `from` / `to`<br>`connector_id` / `confirm` | <strong>参加者の ID を付け替える。</strong>上の 7 か所を 1 つのトランザクションで直す。`confirm` に旧 ID を求める。**待受け（接続）が走っている間は断る**——付け替えても、その待受けは古い ID で poll し続けて新しいカーソルを見ないため。新しい ID が既に使われていれば断る（**片付け済みの行も主キーの枠を占めるので「使われている」とみなす**）。既定のルームへ知らせを積む（[i260909-02](../40_issues/issues.html#i260909-02)） |
...
| C# 版（`aichat-cs`） | `src/cli-cs/` | 24.89 MB | Windows だけ |

> [!CAUTION]
> <strong>直すときは 3 本とも直す。</strong>定義は `src/client/options.mjs` の 1 か所から配るが、振る舞いは写しになる。**食い違いは目で読んで気づける差ではない。**<br> 守るのは 2 つのテスト。`tests/cli-cs.test.mjs` ・ `tests/cli-rs.test.mjs` がサーバーに繋がない分を、`tools/40_test/compare-cli.mjs` が実サーバーへ繋ぐ分を突き合わせる。
...
| `wait` | long-poll。前回の続きから待ち、新着を整形して標準出力に書いて終了する。無ければ待ち直す（既定は最大 12 時間）。**参加・離脱では起きない**（`--with-joins` で含める）。**渡したルームのどれかが初めてなら、案内を出してカーソルを立て、待たずに終わる**（[i260909-01](../40_issues/issues.html#i260909-01)）。実際に待ち始めるのは次の `wait` から |
...
| `rename` | 参加者の ID を付け替える。`connectors` ・ `cursors` ・ `messages`（発言者 ・ 宛先 ・ 本文の `@旧ID`）・ `archives` を横断して直す。先に件数を出し、旧 ID の入力を求める。**待受けが走っている間は断る**（[i260909-02](../40_issues/issues.html#i260909-02)） |
...
ai-chat-lite/
  ...
  src/
    ...
    client/options.mjs             … コマンドとオプションの定義。3 本の CLI の出どころ
    ...
    cli-cs/                        … C# 版 CLI の原本（Args・Commands・Waiters ほか 10 本）
...
| `src/scripts/20_migrate/ver_000006/` | `messages_ix_room_id_sent_at`。`recent --since` / `--before` の絞り込みが効くようにする（[i260830-03](../40_issues/issues.html#i260830-03)） |
