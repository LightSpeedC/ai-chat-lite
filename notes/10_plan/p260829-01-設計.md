...
| 在席表示 | 誰がいまオンラインかが分かる |
| AI の待受け | <strong>親のセッションが背面（`run_in_background`）で常駐させ、着信に反応する。</strong>当初はサブエージェントを想定していたが、挟むこと自体が資源を食うと実測で分かり改めた（[i260905-04](../40_issues/issues.md#i260905-04)） |
...
| [i260920-01 初回の起点を早める](i260920-01-初回の起点を早める.md) | 初めての待受けの起点を「参加した時点」から「その日の 0 時と 6 時間前の古い方」に変えた。**先行実装でテストスイートが連鎖的に壊れたため一度差し戻し、テストごとに専用ルームを使う対策をまとめてから再実装**。[i260909-01](../40_issues/issues.md#i260909-01) の案内文も新しい起点に合わせて書き換えた | ✅ **済** | [API](#4-api) / [クライアントと待受け](#6-クライアントと待受け) |
...
| GET | `/api/cursor-status` | `connector_id` / `room_id`（カンマ区切り可） | 渡したルームのどれかで初めての接続なら `first_time: true`。**読むだけで `setCursor` を呼ばない**（poll と違い進めない）。`wait` の起動時の案内（[i260909-01](../40_issues/issues.md#i260909-01)）に使う |
...
| POST | `/api/admin/rename` | `from` / `to`<br>`connector_id` / `confirm` | <strong>参加者の ID を付け替える。</strong>上の 7 か所を 1 つのトランザクションで直す。`confirm` に旧 ID を求める。**待受け（接続）が走っている間は断る**——付け替えても、その待受けは古い ID で poll し続けて新しいカーソルを見ないため。新しい ID が既に使われていれば断る（**片付け済みの行も主キーの枠を占めるので「使われている」とみなす**）。既定のルームへ知らせを積む（[i260909-02](../40_issues/issues.md#i260909-02)） |
...
> [!CAUTION]
> <strong>直すときは 2 本とも直す。</strong>定義は `src/client/options.mjs` の 1 か所から配るが、振る舞いは写しになる。**食い違いは目で読んで気づける差ではない。**<br> 守るのは `tests/cli-rs.test.mjs`（サーバーに繋がない分）と `tools/40_test/compare-cli.mjs`（実サーバーへ繋ぐ分）。**かつて C# 版もあり（[i260918-01](../40_issues/issues.md#i260918-01) で削除）、3 本の突き合わせだった。**
...
| `wait` | long-poll。前回の続きから待ち、新着を整形して標準出力に書いて終了する。無ければ待ち直す（既定は最大 12 時間）。**参加・離脱では起きない**（`--with-joins` で含める）。**渡したルームのどれかが初めてなら、案内を出してカーソルを立て、待たずに終わる**（[i260909-01](../40_issues/issues.md#i260909-01)）。実際に待ち始めるのは次の `wait` から |
...
| `rename` | 参加者の ID を付け替える。`connectors` ・ `cursors` ・ `messages`（発言者 ・ 宛先 ・ 本文の `@旧ID`）・ `archives` を横断して直す。先に件数を出し、旧 ID の入力を求める。**待受けが走っている間は断る**（[i260909-02](../40_issues/issues.md#i260909-02)） |
...
| `src/scripts/20_migrate/ver_000006/` | `messages_ix_room_id_sent_at`。`recent --since` / `--before` の絞り込みが効くようにする（[i260830-03](../40_issues/issues.md#i260830-03)） |
...
> はじめは `store.mjs` の `CREATE TABLE IF NOT EXISTS` を残していたが、**2 つの困りごとが出たのでやめた**。定義が 2 か所にあると食い違っても静かに動くこと、そして<strong>`AICHAT_DATA` を立てずに読み込むと本番の DB にテーブルが作られること</strong>である（[i260902-01](../40_issues/issues.md#i260902-01)）。後者は実際に起き、版 3 を当てられない状態になった。
