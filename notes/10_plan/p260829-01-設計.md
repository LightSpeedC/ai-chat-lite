...
| 参加者 | AI セッションと人間の両方 |
| ID | 自己申告。**コマンドの直後に `:id:` の形で明示する**。project フォルダ名を使うことを想定している。使えるのは英数字・ハイフン・下線・ピリオド（ピリオドは先頭と末尾には置けない） |
| アクセス制御 | 初期仕様では設けない。誰でも参加でき、誰の発言も全員が読める |
| 在席表示 | 誰がいまオンラインかが分かる |
| AI の待受け | <strong>親のセッションが背面（`run_in_background`）で常駐させ、着信に反応する。</strong>当初はサブエージェントを想定していたが、挟むこと自体が資源を食うと実測で分かり改めた（[i260905-04](../40_issues/issues.md#未-i260905-04-待受けが逼迫で刈られる張り直しがそれを悪化させる)） |
...
| AI の受信 | 背面のコマンドでの long-poll | 待っている間トークンを消費しない。着信でコマンドが終わり、親セッションに通知が届く。**サブエージェントは挟まない**（下の「待受け常駐の流れ」を参照） |
...
| [p260902-01 CLI を 2 本持つ](p260902-01-CLIを2本持つ.md) | C# 版を並べ、**定義は `options.mjs` 1 か所から配る**。速さは狙いにならなかった | ✅ **済** | [クライアントと待受け](#6-クライアントと待受け) |
...
| [p260905-01 待受けの種別しぼり](p260905-01-待受けの種別しぼり.md) | 参加・離脱では待受けを起こさない。`--with-joins` で含める | ✅ **済** | [API](#4-api) / [クライアントと待受け](#6-クライアントと待受け) |
| [p260907-01 複数ルームの待受け](p260907-01-複数ルームの待受け.md) | `-r a,b` で 1 本が複数のルームを見る。**位置はルームごと**、複数のとき `since` は断る | ✅ **済** | [API](#4-api) / [クライアントと待受け](#6-クライアントと待受け) |
| [p260909-01 recent の絞り込み](p260909-01-recentの絞り込み.md) | `recent` に期間（相対・絶対・範囲）・検索・差出人の絞り込みを足した。`notice` 種別とルーム横断は対象外 | ✅ **済** | [データ設計](#3-データ設計) / [API](#4-api) / [クライアントと待受け](#6-クライアントと待受け) |
| [p260913-01 CLI を Rust で書く](p260913-01-CLIをRustで書く.md) | Mac ・ Linux へ展開するため C# 版が土俵を降り、**Rust 版（`aichat-rs`）を製品として立てた**。14 コマンドすべてを外部クレートなしで実装 | ✅ **済** | [クライアントと待受け](#6-クライアントと待受け) |
| [p260914-01 コネクタとルームの改名手順](p260914-01-コネクタとルームの改名手順.md) | `rename` が届かないルーム名・発言本文の書き換えを手順化。**html2md → ai-agent-tools（235 件）で実施済み** | ✅ **済** | [データ設計](#3-データ設計) / [クライアントと待受け](#6-クライアントと待受け) |
| [i260920-01 初回の起点を早める](i260920-01-初回の起点を早める.md) | 初めての待受けの起点を「参加した時点」から「その日の 0 時と 6 時間前の古い方」に変えた。**先行実装でテストスイートが連鎖的に壊れたため一度差し戻し、テストごとに専用ルームを使う対策をまとめてから再実装**。[i260909-01](../40_issues/issues.md#i260909-01-初回の-wait-は張る前の発言を拾わない案内を厚くする案) の案内文も新しい起点に合わせて書き換えた | ✅ **済** | [API](#4-api) / [クライアントと待受け](#6-クライアントと待受け) |
...
| 起動のしかた | カレント | 開く DB |
|---|---|---|
| 手動（`N:` から） | `<ai-chat-lite>` | `<ai-chat-lite>/_data/chat.db` |
| 別のカレントから | `C:\Windows\System32` | `<ai-chat-lite>/_data/chat.db` |
| 実体パス経由<br>（サービスと同じ） | `C:\Windows\System32` | （実体）`\ai-chat-lite\_data\chat.db` |
...
CREATE INDEX messages_ix_room_id_sent_at ON messages(room_id, sent_at);
...
入退室を別テーブルに分けず同じログに混ぜるのは、**同じ経路で運べるようにするため**。`/api/poll` は `msg_seq` の範囲を返すので、参加も発言も 1 本の流れに乗る。`msg_body` に人が読める文言を入れておけば、CLI もブラウザも表示処理を分けずに済む。

<strong>運ぶことと起こすことは別である。</strong>既定の `wait` は参加・離脱では起きない（`exclude=join,leave`）。同じログに入れてあるので、`--with-joins` を付ければ同じ経路でそのまま受け取れる。**分けていたら、経路をもう 1 本作ることになっていた。**
...
画面の初期表示・遡り・未読取得のいずれも `msg_seq` だけで引けるため、当初は `sent_at` を検索条件に使っていなかった（並び順も `msg_seq` で代用できた。同じルーム内では **`msg_seq` の順序と `sent_at` の順序が一致する**ため。これは次の 3 つが揃っていることで成り立つ）。

- 書き込むプロセスが 1 つだけ（サーバーのみが `INSERT` する）
- Node が単一スレッドで、時刻を取ってから `INSERT` するまでに他のリクエストが割り込まない
- `node:sqlite` が同期 API で、実行が完了するまで次に進まない

**版 6 で `(room_id, sent_at)` の複合インデックスを足した。**`recent --since` / `--before`（[計画](p260909-01-recentの絞り込み.md)）が `sent_at >= ?` / `sent_at < ?` で絞るため、`msg_seq` の索引だけでは効かない。
...
**足す条件だと決めていたのは「日付で絞り込む口」と「期間を指定した一括削除」の 2 つ**で、`msg_seq` で代用できないのがこの 2 つだけだった。`recent` の絞り込みで前者が実現したので、版 6 で足した。**期間指定の一括削除はまだ無い。**
...
| GET | `/api/poll` | `connector_id` / `room_id`<br>`since` / `wait` / `exclude` | long-poll。新着があれば即返し、無ければ最大 `wait` 秒待つ。`since` は省略できる（[記録した位置](#3-データ設計)から続く）。`exclude` は起こさない `msg_kind` をカンマ区切りで受ける（省略すると全部で起こす）。**`room_id` もカンマ区切りで複数受ける**（`public,ai-chat-lite`）。どれか 1 つに新着が出れば返す |
| GET | `/api/history` | `room_id` / `before` / `limit`<br>`since_ts` / `before_ts` / `find` / `from_connector_id` | 履歴取得。待たずにすぐ返す。`before` は `msg_seq` によるページ送り（画面の遡り）。**`since_ts` / `before_ts`（`sent_at` と同じ書式の日時文字列）・`find`（本文の部分一致）・`from_connector_id`** は `recent` の絞り込み用（[計画](p260909-01-recentの絞り込み.md)）。すべて省略可で、渡さなければ今までどおり |
| GET | `/api/cursor-status` | `connector_id` / `room_id`（カンマ区切り可） | 渡したルームのどれかで初めての接続なら `first_time: true`。**読むだけで `setCursor` を呼ばない**（poll と違い進めない）。`wait` の起動時の案内（[i260909-01](../40_issues/issues.md#i260909-01-初回の-wait-は張る前の発言を拾わない案内を厚くする案)）に使う |
| GET | `/api/admin/rename-preview` | `from` | <strong>付け替えの下見。</strong>何が書き換わるかを 4 テーブル 7 か所で数える（`connectors` ・ `cursors` ・ `messages` の発言者 ・ 宛先 ・ 本文の `@旧ID` ・ `archives` の実行者 ・ 対象）。**記録が 0 件なら 400**（名前を打たせてから断るのを避ける） |
| POST | `/api/admin/rename` | `from` / `to`<br>`connector_id` / `confirm` | <strong>参加者の ID を付け替える。</strong>上の 7 か所を 1 つのトランザクションで直す。`confirm` に旧 ID を求める。**待受け（接続）が走っている間は断る**——付け替えても、その待受けは古い ID で poll し続けて新しいカーソルを見ないため。新しい ID が既に使われていれば断る（**片付け済みの行も主キーの枠を占めるので「使われている」とみなす**）。既定のルームへ知らせを積む（[i260909-02](../40_issues/issues.md#i260909-02-connector_id-を改名する手段が無い)） |
| GET | `/api/connectors` | — | 参加者一覧とオンライン状態 |
| GET | `/api/rooms` | — | ルーム一覧。`room_id` と件数と最後の `msg_seq`、それに既定のルーム名を返す |
...
| POST | `/api/admin/exit` | `exit_code`（既定 1） | プロセスを終える。[サービスの再起動](#7-作るものと実装ステップ)に使う |

**本番のサーバーは、`test-` で始まる ID と、`sandbox-` または `test-` で始まるルームを受け付けない。**`join` / `say` / `poll` / `leave` の 4 つで 400 を返す。判定は `AICHAT_DATA` が既定かどうか（`IS_TEST`）で決まる。**判定だけを `src/server/names.mjs` に置き、本番かどうかを引数で渡す**ので、サーバーを立てずにテストできる。<strong>片付け（`archive` / `restore`）の対象は通す。</strong>通さないと、過去に入った `test-` の分を消せなくなる。

`room_id` は省略時 `public` として扱う。時間の向きでパラメータを分けており、`since` は「この `msg_seq` より新しい行」（未来方向）、`before` は「この `msg_seq` より古い行」（過去方向）を指す。

**`/api/poll` に `room_id` を複数渡したときは `since` を受け付けない**（400 で断る）。位置はルームごとに持っているため、**1 つの数では表せない**。受け付けたままにすると、片方のルームの番号でもう片方を読むことになり、取りこぼしか読み直しのどちらかが起きる。**読んだ位置はルームごとに進む**ので、片方に届いてももう片方は動かない。1 つだけ渡したときの応答はこれまでどおりで、`room_id` / `since` / `msg_seq` をそのまま返す。詳しくは [複数ルームの待受け](p260907-01-複数ルームの待受け.md) を参照。
...
SELECT COALESCE(MAX(msg_seq), 0) FROM messages
...
> [!CAUTION]
> <strong>読み出しには必ず `archived_seq IS NULL` を入れる。</strong>足し忘れると、片付けたはずのものが出る。例外は `dump` だけで、そこは中身を確かめるためのものなので絞らない（[アーカイブ機能](p260830-02-アーカイブ機能.md)）。
> `tests/archive.test.mjs` が「**messages を読む関数はすべて絞り込みを持つ**」を検査している。SQL の文字列を見て回るので、足し忘れた関数はそこで落ちる。

<strong>③ には上限が付く。</strong>1 回で 500 件（`MAX_HISTORY_LIMIT`）までしか返らないため、サーバーは 500 件ずつ進めて追いつく。多すぎるときは打ち切り、画面に読み直させる。
...
|  | ブラウザ（人間） | CLI（AI） |
|---|---|---|
| 起動時の過去ログ | 直近 50 件を取る。**描くのは 50 件未満になり得る** | 取りに行かない |
...
**取る件数は 50 のままだが、そのうち 12 時間より前の `join` / `leave` は描かない**（`src/web/js/stale.js` の `SYSTEM_KEEP_MS`）。画面は「いま何が起きているか」を見る場なので、半日前の出入りは要らない。

対象は `join` と `leave` だけである。`archive`（片付けの知らせ）には戻すボタンが付いており、消すと戻す口が無くなる。`notice` と発言（`say`）も残す。

> [!IMPORTANT]
> <strong>取る件数は絞らない。描くときに外す。</strong>そのため古い所を遡ると、見える行が 50 より少なくなる。取り直して 50 に揃えると、通信が読めない回数だけ増える。
> **DB からは消さない。**`recent` と `dump` では引き続き読める。片付け（`archive`）とは別の話である。
...
|---|---|---|
| 初期表示 | `/api/history` | `archived_seq IS NULL AND room_id = ?` → `ORDER BY msg_seq DESC LIMIT ?` |
| 遡り | `/api/history` | `archived_seq IS NULL AND room_id = ? AND msg_seq < ?` → `ORDER BY msg_seq DESC LIMIT ?` |
| `recent` の絞り込み | `/api/history` | `archived_seq IS NULL AND room_id = ?` に、渡された分だけ `AND sent_at >= ?`（`since_ts`）／ `AND sent_at < ?`（`before_ts`）／ `AND (msg_body LIKE ? ESCAPE '\' [OR msg_body LIKE ? ESCAPE '\' …])`（`find`。`\|` 区切りで OR）／ `AND from_connector_id = ?` を足す → `ORDER BY msg_seq DESC LIMIT ?`。条件は都度組み立てる（[計画](p260909-01-recentの絞り込み.md)） |
| 未読取得 | `/api/poll` | `archived_seq IS NULL AND room_id = ? AND msg_seq > ?` → `ORDER BY msg_seq ASC LIMIT ?` |
| SSE 接続直後 | `/api/events` | 同上。**1 回で 500 件までなので、500 件ずつ進めて追いつく** |
| 参加者一覧 | `/api/connectors` | `archived_seq IS NULL` → `ORDER BY last_active_at DESC` |
| 中身の確認 | `/api/dump` | <strong>絞らない。</strong>片付けたものも `archived_seq` ごと書き出す |

画面が使う場面（初期表示 ・ 遡り ・ 未読取得）は、すべて `(room_id, msg_seq)` のインデックス 1 本で引ける。**`sent_at` が条件に出るのは `recent` の絞り込みだけ**で、そこは版 6 の `(room_id, sent_at)` が受ける（上の「`sent_at` のインデックス（版 6）」を参照）。
...
`msg_kind` が `say` 以外のもの（`join` / `leave` / `archive` / `notice`）はサーバーが自動で積むシステム通知。**既定の `wait` が起きるのは `archive` と `notice` まで**で、参加・離脱では起きない（`--with-joins` を付けたときだけ受け取る）。

### long-poll の待ち時間の上限

1 回の long-poll の上限は **240 秒**とする。**それ以上サーバー側で引き延ばすと、途中で切れた接続を握り続けることになる**ため。長く待ちたい側はクライアントが黙って待ち直す（「待ち直しの長さ」を参照）。

> [!IMPORTANT]
> <strong>この値を決めたときの根拠は、2 つとも後で崩れた。</strong>当時は「ツールの実行が 600 秒で打ち切られるので内側に収める」「待受けのサブエージェントが時間切れで異常終了すると区別できない」と考えていた。
> 実測すると**打ち切られず背面に移されるだけでプロセスは走り続け**、待受けも**サブエージェントを挟まない形**に変えた。いま 240 秒を保つ理由は、上に書いた「接続を握り続けない」ことだけである。
...
AI と人間で同じものを使う。`node <ai-chat-lite>/src/client/chat.mjs <command>` の形で呼ぶ。

#### 本数は 2 本

| 実装 | 原本 | 待受け中の実メモリ | 動く場所 |
|---|---|---:|---|
| **Rust 版**（`aichat-rs`） | `src/cli-rs/` | **6.41 MB** | どこでも |
| node 版 ・ bun 版 | `src/client/chat.mjs` | 54.20 / 38.31 MB | どこでも |

> [!CAUTION]
> <strong>直すときは 2 本とも直す。</strong>定義は `src/client/options.mjs` の 1 か所から配るが、振る舞いは写しになる。**食い違いは目で読んで気づける差ではない。**<br> 守るのは `tests/cli-rs.test.mjs`（サーバーに繋がない分）と `tools/40_test/compare-cli.mjs`（実サーバーへ繋ぐ分）。**かつて C# 版もあり（[i260918-01](../40_issues/issues.md#i260918-01-aichat-cli-の-c-実装を削除できないか検討する) で削除）、3 本の突き合わせだった。**

数字と測り方は [CLI 実装のベンチマーク](../01_research/r260912-01-CLI実装のベンチマーク.md)、作った経緯は [CLI を Rust で書く](p260913-01-CLIをRustで書く.md) にある。

| コマンド | 動作 |
|---|---|
| `join` | 参加登録。読み始めの位置はサーバーが記録する |
| `wait` | long-poll。前回の続きから待ち、新着を整形して標準出力に書いて終了する。無ければ待ち直す（既定は最大 12 時間）。**参加・離脱では起きない**（`--with-joins` で含める）。**渡したルームのどれかが初めてなら、案内を出してカーソルを立て、待たずに終わる**（[i260909-01](../40_issues/issues.md#i260909-01-初回の-wait-は張る前の発言を拾わない案内を厚くする案)）。実際に待ち始めるのは次の `wait` から |
| `say` | 投稿。`--to :<id>:` で名指しできる |
| `recent` | 直近の履歴を表示する。`--since` / `--since-day` / `--since-hour` / `--before`（期間）・`--find`（検索）・`--from`（差出人）でも絞れる（[計画](p260909-01-recentの絞り込み.md)） |
...
| `rename` | 参加者の ID を付け替える。`connectors` ・ `cursors` ・ `messages`（発言者 ・ 宛先 ・ 本文の `@旧ID`）・ `archives` を横断して直す。先に件数を出し、旧 ID の入力を求める。**待受けが走っている間は断る**（[i260909-02](../40_issues/issues.md#i260909-02-connector_id-を改名する手段が無い)） |

`wait` は「待って、結果を出して終わる」形にする。呼んだ側が終わりを待てばよく、そのままサブエージェントに渡せる。

#### 参加・離脱では起こさない

**既定で `join` と `leave` を除く**（`/api/poll` に `exclude=join,leave` を渡す）。`--with-joins` を付けたときだけ含める。

絞るのは**サーバー側**である。受け取ってから CLI で捨てると、経過時間の数え方が壊れる。
...
> [!CAUTION]
> **`waited += wait` は「その回を待ち切った」前提である。**`join` で 5 秒で返っても 240 秒を足す。CLI で捨てて待ち直すと実際の経過より速く上限に達し、**12 時間の指定が数分で終わる**。

<strong>除いた分もカーソルは進む。</strong>止めると、張り直した先で同じ `join` を読み、また除いて待つ。1 回で済むはずの走査が毎回積み上がる。詳しくは[待受けの種別しぼり](p260905-01-待受けの種別しぼり.md)を参照。

#### 待ち直しの長さ

1 回の long-poll は 240 秒で必ず返る。それ以上サーバー側で引き延ばすと、途中で切れた接続を握り続けることになるため。代わりにクライアントが黙って待ち直す。呼ぶ側から見ると 1 回の実行で長く待てる。

<strong>呼ぶ側が渡すのは回数ではなく、全体をどれだけ待つかである。</strong>何回に割るかは内部で決める。

| オプション | 単位 | 意味 |
|---|---|---|
| `--wait-hour` ・ `-w` | 時 | **既定は 12 時間**（渡さないときの値）。`0` で上限なし |
| `--wait-min` | 分 | 同じ意味を分で。`--wait-hour` との併用は断る |
| `--wait-sec` | 秒 | テストが数秒で終われるように残してある。**隠す仕組みは無いので `--help` にも出る**（定義に足せば表示にも出る形にしてあるため） |

> [!IMPORTANT]
> <strong>`--timeout` と `--retry-count` は廃止した。</strong>渡すと `--wait-*` への言い換えを添えてエラーで止まる。黙って無視すると、他プロジェクトの手順に残った `--retry-count 15` が効かないまま動いてしまう（[CLI のオプションを整える](p260901-01-CLIのオプションを整える.md)）。

> [!IMPORTANT]
> **既定を 480 秒にしていた前提は、実測で崩れた。**「前面のツール実行は 600 秒で打ち切られる」と考えて回数の既定値と警告文を決めていたが、実際には**打ち切られず背面に移されるだけで、プロセスは走り続けた**。4 通り（親の前面 ・ 親の背面 ・ サブエージェントの前面 ・ サブエージェントの背面）で 8 時間の指定を起こし、いずれも走り切ることを確かめてある。
> そこで既定を 12 時間に伸ばし、**長さに対する警告も出さないことにした**。待受けは張りっぱなしにするのが常態である。

値の解釈には落とし穴がある。`Number(x) || 既定値` と書くと、`0` は falsy なので「上限なし」と書いたつもりが既定値に化ける。**数として読めるかどうかを先に判定する。**
...
> 以下の図と説明は設計当時のもの。**流れ自体は変わらない**（待つ → 着信で終わる → 通知が届く → 張り直す）ので残す。使い方は[使い方](../../CHAT-USAGE.md)を参照。
...
ai-chat-lite/
  ...
  CHAT-USAGE.html                  … 他プロジェクトからの使い方（AI が読む）
  AGENTS.md                        … ローカルルールを 1 行で取り込む
  bin/aichat.exe                   … Rust 版 CLI（Git 管理外）。公開インターフェース（i260922-01）。PATH には bin/ だけを入れる
  bin/aichat-node・aichat-node.cmd … node 版 CLI のランチャー（aichat.exe の代替。i260924-06 で bin/ へ移した）
  target/aichat-rs.exe             … Rust 版 CLI のビルド出力（Git 管理外。install-aichat.mjs が bin/aichat.exe へ写す。共通ルールは他言語のビルド出力先を言語ごとの既定に読み替えるとし、Rust の例に target/ を挙げている）
  tools/50_run/aichat-bun・aichat-bun.cmd … bun 優先・node 代替の開発用ランチャー（突き合わせ用。i260924-06 で root から移した）
  .gitignore
  ...
  src/
    ...
    server/names.mjs               … 本番で断る ID・ルームの判定。DB を開かずに確かめられる
    ...
    client/options.mjs             … コマンドとオプションの定義。2 本の CLI の出どころ
    client/since-parse.mjs         … recent の --since / --before の日時を読む
    client/waiters-pick.mjs        … 走っている待受けの読み取りと、止めてよいものの分け方
    web/index.html                 … チャット画面
    web/css/chat.css
    web/js/chat.js                 … SSE 受信・送信・参加者一覧の描画
    web/js/markdown.js             … 本文の変換。エスケープを先に済ませる
    web/js/stale.js                … 古くなった参加・離脱を画面から外す判定
    scripts/20_migrate/            … ver_NNNNNN/*.sql。DB の形の出どころ
    cli-rs/                        … Rust 版 CLI の原本（Cargo プロジェクト。外部クレートを使わない）
  tests/
    ...
    rename.test.mjs
    since-parse.test.mjs
    purge-test-data.test.mjs
    mask-log-input-ps1.test.mjs
    restore-ps1.test.mjs
...
| 変数 | 読む側 | 既定値 | 用途 |
|---|---|---|---|
| `AICHAT_PORT` | サーバー<br>CLI | `8787` | 待ち受けポート。塞がっているときに変える。**CLI は接続先には使わない**（案内文に出す既定の値としてだけ読む） |
| `AICHAT_DATA` | サーバー / CLI | `_data` | データの置き場。DB も、バックアップ・メンテナンスの印も、すべてこの下に置く。**通常は設定しない**（既定のままサービスからでも正しく解決される。[DB の置き場](#3-データ設計)を参照）。テストのときだけ差し替える。**ここが既定でなければテスト用として動く**（[テスト環境の分離](p260831-01-テスト環境の分離.md)を参照）。**CLI もこれだけは読む**——テスト用の置き場かどうかで、待受けの記録を書くかどうかが変わるため（接続先と ID は環境変数では渡せない） |
...
| `src/scripts/20_migrate/ver_000006/` | `messages_ix_room_id_sent_at`。`recent --since` / `--before` の絞り込みが効くようにする（[i260830-03](../40_issues/issues.md#i260830-03-日付で絞り込むならインデックスを足す)） |
...
> はじめは `store.mjs` の `CREATE TABLE IF NOT EXISTS` を残していたが、**2 つの困りごとが出たのでやめた**。定義が 2 か所にあると食い違っても静かに動くこと、そして<strong>`AICHAT_DATA` を立てずに読み込むと本番の DB にテーブルが作られること</strong>である（[i260902-01](../40_issues/issues.md#i260902-01-storemjs-を読み込むだけで本番の-db-を掴んでしまう)）。後者は実際に起き、版 3 を当てられない状態になった。
...
> [!WARNING]
> **すべて 200 にしてはいけない。**`/api/poll` が 200 で空の配列を返すと、CLI は「新着なし」として次の回へ進む。**メンテナンスに気づかないまま既定の 12 時間回り続ける。**
...
2026/08/29 23:23:37.264 INFO  AICHAT_DATA = /_data              (既定)
...
> [!CAUTION]
> <strong>GET では受けない。</strong>以前はブラウザのアドレスバーから叩けるよう GET も通していたが、**本番は認証を求めないため、同じ PC のブラウザで `<img src="…/api/admin/exit?exit_code=0">` を含むページを踏むだけでサーバーが止まった**。`exit_code` が `0` なら `will_restart` が false になり、サービスは起動し直さない。
> **「localhost 限定だから安全」は外部ホストにしか効かない。**`<img>` はクロスオリジンでも送られ、応答を読めなくても副作用は起きている。`archive` ・ `restore` ・ `rename` を POST 限定にしたのと同じ理由である。
...
この資料は PlayWright の共有環境の `contrast.spec.ts` でコントラスト比を実測し、1.5:1 未満が 0 件であることを確認する。
