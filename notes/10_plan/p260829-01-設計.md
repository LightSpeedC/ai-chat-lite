...
| GET | `/api/poll` | `connector_id` / `room_id`<br>`since` / `wait` / `exclude` | long-poll。新着があれば即返し、無ければ最大 `wait` 秒待つ。`since` は省略できる（[記録した位置](#3-データ設計)から続く）。`exclude` は起こさない `msg_kind` をカンマ区切りで受ける（省略すると全部で起こす） |
