...
**本番のサーバーは、`test-` で始まる ID と `sandbox-` で始まるルームを受け付けない。**`join` / `say` / `poll` / `leave` の 4 つで 400 を返す。判定は `AICHAT_DATA` が既定かどうか（`IS_TEST`）で決まる。**判定だけを `src/server/names.mjs` に置き、本番かどうかを引数で渡す**ので、サーバーを立てずにテストできる。<strong>片付け（`archive` / `restore`）の対象は通す。</strong>通さないと、過去に入った `test-` の分を消せなくなる。
