...
| GET | `/api/admin/rename-preview` | `from` | <strong>付け替えの下見。</strong>何が書き換わるかを 5 テーブル 7 か所で数える（`connectors` ・ `cursors` ・ `messages` の発言者 ・ 宛先 ・ 本文の `@旧ID` ・ `archives` の実行者 ・ 対象）。**記録が 0 件なら 400**（名前を打たせてから断るのを避ける） |
