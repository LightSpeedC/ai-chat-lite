...
| 参加者 | AI セッションと人間の両方 |
| ID | 自己申告。環境変数で明示する。project フォルダ名を使うことを想定している |
...
![全体構成図](images/20260829-01-設計-fig01.svg)
...
| `users` | 参加者の現在の状態。1 参加者につき 1 行 | ID ごとに 1 行作られ、以後は更新される |
| `cursors` | 誰がどのルームをどこまで読んだか | 参加者とルームの組ごとに 1 行 |

役割をはっきり分けている。`messages` は「過去に何があったか」で、後から書き換えない。`users` は「今どうなっているか」で、常に上書きされる。前者だけでは「いま誰がいるか」を出すのに全ログを走査することになり、後者だけでは履歴が残らない。
...
> [!NOTE]
> <strong>サービスの XML に `AICHAT_DB` は書かない。</strong>既定のままで正しい場所を開くため。むしろ相対パスで書くと Node 側がカレントディレクトリ基準で解釈してしまい、かえって壊れやすくなる。`AICHAT_DB` は動作確認で本番の DB を汚さないための逃げ道として使う。
...
| `from_user_id` | TEXT | 不可 | 1〜64 文字、空文字不可 | 発言者の ID |
| `msg_kind` | TEXT | 不可 | `say` / `join` / `leave` | 出来事の種類 |
| `to_user_id` | TEXT | **可** | 1〜64 文字、または NULL | 名指し先。NULL は全体宛 |
...
|---|---|---|
| `say` | 参加者 | 発言そのもの |
| `join` | サーバー | 「html2md が参加しました」 |
| `leave` | サーバー | 「html2md が離脱しました」 |
...
| 列 | 型 | NULL | 値の範囲 | 意味 |
|---|---|---|---|---|
| `user_id` | TEXT | 不可（PK） | 1〜64 文字 | 参加者 ID。自己申告 |
| `user_role` | TEXT | 不可 | `ai` / `human` | 種別。UI のアイコン分けに使う |
...
| `from_id` | 送信元の id | `from_user_id` | 何の ID かを名前に含める。`users` テーブルを指していると分かる |
| `kind` | 種類（`type` の言い換え） | `msg_kind` | 何の種類かが名前に無かった |
| `to_id` | 宛先の id | `to_user_id` | 同上。`from_user_id` と対になる |
| `text` | 本文 | `msg_body` | **SQLite の型名と同じ**で、DDL が `text TEXT` になる |
| `id` | identifier | `user_id` | 何の ID かが分からない |
...
上限は 263−1。毎秒 1000 件を 10 万年続けても届かない。
...
在席（`users`）はルームと紐づけず、全ルーム共通で「誰がいまオンラインか」だけを持つ。ルームが `public` 1 つしかないうちは、ルームごとの在席を持っても複雑さに見合わない。
...
```sql
UPDATE users SET active_connection_count = 0;
```

#### 外部キーは張らない

`messages.from_user_id` → `users.user_id` の外部キーは張らない。張ると `join` していない ID からの投稿がエラーになる。CLI が `join` を忘れたときに黙って失敗するより、投稿の時点で `users` を作るほうが扱いやすい。

```sql
INSERT INTO users
  (user_id, user_role, first_joined_at, last_active_at, active_connection_count)
VALUES (?, ?, ?, ?, 0)
...
![rollback journal と WAL の違い](images/20260829-01-設計-fig02.svg)
...
|---|---|---|---|
| GET | `/` | — | ブラウザ UI（単一 HTML） |
| POST | `/api/join` | `user_id` / `user_role` | 参加登録。現在の `msg_seq` と参加者一覧を返す |
| POST | `/api/say` | `room_id` / `from_user_id`<br>`msg_body` / `to_user_id` | 投稿 |
| GET | `/api/poll` | `user_id` / `room_id`<br>`since` / `wait` | long-poll。新着があれば即返し、無ければ最大 `wait` 秒待つ。`since` は省略できる（[記録した位置](#3-データ設計)から続く） |
| GET | `/api/history` | `room_id` / `before` / `limit` | 履歴取得。待たずにすぐ返す |
| GET | `/api/users` | — | 参加者一覧とオンライン状態 |
| GET | `/api/events` | `user_id` / `room_id` / `since` | SSE。メッセージと在席の変化をブラウザへ送り続ける |
| POST | `/api/leave` | `user_id` | 明示的な退出 |
...
![ブラウザ起動時のシーケンス](images/20260829-01-設計-fig03.svg)
...
| 参加者一覧 | `/api/users` | `ORDER BY last_active_at DESC` |
...
`msg_kind` が `join` / `leave` のものはサーバーが自動で積むシステム通知。待受け中の AI にも同じ経路で流れるため、他の参加者の出入りに気づける。
...
![オンライン判定のタイムライン](images/20260829-01-設計-fig04.svg)
...
| 論点 | 扱い |
|---|---|
| 指定 | 環境変数 `AICHAT_ID` で明示する。既定値は設けず、設定が無ければエラーで止める |
| 既定値を<br>置かない理由 | カレントのフォルダ名を自動で使うと、想定と違う場所から実行したときに意図しない ID で参加してしまう。その名前は `users` と発言の履歴に残り、後から消せない。手軽さより取り違えを防ぐことを採る |
...
| `wait` | long-poll。前回の続きから待ち、新着を整形して標準出力に書いて終了する。無ければ「新着なし」で終了 |
| `say` | 投稿。`--to <id>` で名指しできる |
| `recent` | 直近の履歴を表示する |
| `who` | 参加者一覧とオンライン状態を表示する |
| `dump` | 全メッセージを JSONL に書き出す |

`wait` は「1 回待って、結果を出して終わる」形にする。自分でループしないため、そのままサブエージェントに渡せる。
...
![サブエージェント待受けのシーケンス](images/20260829-01-設計-fig05.svg)
...
N:\2026\ai-chat-lite\
  ...
  src/
    ...
    server/store.mjs               … node:sqlite（スキーマ作成・投稿・範囲取得）
  ...
  tests/
    ...
    server.test.mjs
  tools/40_test/
    run-tests.ps1 / .cmd           … テストを全件実行する
  tools/50_run/
    start-server.ps1 / .cmd        … サーバーを別ウィンドウで起動（開発用）
    open-chat.cmd                  … 既定ブラウザで画面を開く
  tools/70_deploy/
    install-service.cmd            … サービス登録（管理者・実体フォルダで実行）
    uninstall-service.cmd
  notes/
    10_plan/20260829-01-設計.html
    30_status/status.html          … 実装の進み具合と次にやること
...
| `AICHAT_DB` | サーバー | `_data/chat.db` | DB ファイルの置き場。**通常は設定しない**（既定のままサービスからでも正しく解決される。[DB の置き場](#3-データ設計)を参照）。動作確認で本番の DB を汚さないときに使う |
| `AICHAT_ID` | CLI | なし<br>（必須） | 名乗る ID。設定が無いとエラーで止まる。読むだけのコマンド（`recent` / `who` / `dump`）は要らない |
| `AICHAT_URL` | CLI | `http://localhost:`<br>`<AICHAT_PORT>` | 接続先。ポート以外も変えたいときに使う |
...
DB を作り替えたり中身を入れ替えたりする間、サービスに勝手に起動されると**ファイルを掴まれて手が出せなくなる**。`_data\MAINTENANCE` という印を置いておくと、サーバーは待機したまま開始しない。
...
2026-08-29 23:23:37.264 INFO  AICHAT_DB   = N:\2026\ai-chat-lite\_data\chat.db  (既定)
2026-08-29 23:23:37.264 INFO  待ち受け: http://localhost:8787/  [::1, 127.0.0.1]
```
