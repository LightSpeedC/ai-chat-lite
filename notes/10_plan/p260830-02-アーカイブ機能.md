...
動作確認で作った参加者やルーム、誤って投稿した発言を片付ける手段が無かった。**DB を直接触るのは危険**なので、CLI と API から片付けられるようにした。他のプロジェクトからも使えるよう [使い方](../../CHAT-USAGE.md) に書く。
...
##### 繋ぎ直しで自然に復活した分は、戻しても 0 件になる

**参加者は繋ぎ直すと `archived_seq` が自動で `NULL` に戻る**（`upsertConnector`・`touchConnector`・`upsertCursor`）。片付けは ID を使えなくすることではなく、過去のものを一覧から隠すためのものであるため。

> [!IMPORTANT]
> **この状態で古い `archived_seq` を `restore` すると、3 テーブルのどこにも対象行が無い。**`--with-messages` を付けずに片付けた場合は発言も最初から対象外なので、`restore` は 3 テーブルとも 0 件のまま、`archives` の記録だけを消す。
> **これは受け入れる。コードは変えない。**`restore` は既定ルームへ「0 件」の知らせを投稿し、サーバーのログにも残るため、経緯は追える。すでに生きているものを重ねて戻しても実害は無く、`restore` の役目（片付けた記録を片付ける）は果たしている。
