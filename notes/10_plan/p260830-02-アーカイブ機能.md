...
✅ **実装済み** **2026-09-02 に実装した。**`archives` テーブルは版 3 で入れ、CLI の `archive` / `archives` / `restore` と、`/api/admin/*`・`/api/dump` が動いている。本番 DB への適用はまだ。
...
tests/server.test.mjs       … history には出ず dump には出ること
USAGE-FOR-PROJECTS.html     … 他プロジェクトからの使い方
```

`src/web/js/chat.js` は**手を付けていない**。画面から片付ける手段は置かない方針のままで、戻す側を画面に置くのは後回しにした。
