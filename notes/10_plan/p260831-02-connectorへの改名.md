...
| 課題 | 足すもの |
|---|---|
| [i260830-09](../40_issues/issues.md#i260830-09-いらなくなった参加者ルーム発言を片付ける手段がない) | アーカイブ機能。`archives` テーブルと `archived_user_id` |
| [i260831-01](../40_issues/issues.md#未-i260831-01-リアクションと投票がない) | 返信・リアクション・投票。`messages` に列を足す |
...
| `AICHAT_ID` | **変えない**（[i260831-02](../40_issues/issues.md#i260831-02-名乗る-id-を---connector-id-で渡す) で別途扱う） |
...
| 6 | `notice` | <strong>いま足す。</strong>作り直す機会が今回なので、`msg_kind` の `CHECK` を `say` / `join` / `leave` / `archive` / `notice` の 5 つにする（[i260901-03](../40_issues/issues.md#i260901-03-メンテナンスで止めると待受けは理由が分からないまま落ちる)） |
...
[アーカイブ機能](../40_issues/issues.md#i260830-09-いらなくなった参加者ルーム発言を片付ける手段がない)と[返信・リアクション](../40_issues/issues.md#未-i260831-01-リアクションと投票がない)は、**この改名より後にする**。先に作ると、足した列も変えることになる。

ただしアーカイブ機能は**着手中**である。止めてこちらを先にするか、アーカイブを終わらせてから改名するかを決める。

[待受けの時間指定](../40_issues/issues.md#i260901-01-待受けの時間指定を設計からやり直す)も**CLI のオプションを触る**。`--retry-count` を廃止して `--wait-min` / `--wait-hour` を足す課題なので、`--connector-id` と同じ行を書き換えることになる。**どちらを先にするかを決めてから始める。**
