...
| いま | あと | 備考 |
|---|---|---|
| `who` | `connectors` | **「誰が」を尋ねるコマンド。**`/api/connectors` と揃う |
| `--role ai|human` | **変えない** | `role` に `user` は入っていない |
| `--to <id>` | **変えない** | 同上 |
| `join` `wait` `say` `recent` `dump` `leave` | **変えない** | 同上 |
| `USER_ID`（内部の定数） | `CONNECTOR_ID` |  |

`who` だけを変える。**他のコマンドとオプションには `user` が入っていない**ため、触る理由がない。

> [!IMPORTANT]
> <strong>`who` を変えると他プロジェクトの手順が変わる。</strong>USAGE に載っているので、周知の対象になる。
