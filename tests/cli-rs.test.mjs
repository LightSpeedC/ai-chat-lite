 ...
 *
 * 【なぜ要るか】
 * CLI が 3 本（node ・ C# ・ Rust）になった。食い違いは必ず出るし、目で読んで
 * 気づける差ではない。実際に C# 版では、同じ秒に立った待受けの並び順と、
 * say の本文なしの終了コードが node 版と食い違っていた。どちらも
 * tests/cli-cs.test.mjs があって初めて見つかった。
 *
...
const exe = join(root, process.platform === 'win32' ? 'aichat-rs.exe' : 'aichat-rs');
