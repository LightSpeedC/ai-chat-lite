...
> [!IMPORTANT]
> <strong>12 時間を張っても、たいていはもっと早く終わる。</strong>止まるのは空きメモリが減ったときで、`[killed]` として返る。**時間では決まらない**（20 秒で終わることも、数時間走ることもある）。
...
| `[killed]` | **空きメモリが減ると起きる** | <strong>黙って張り直す。</strong>いちばん多い終わり方で、**仕様である** |
...
> <strong>`[killed]` は仕様である。</strong>Claude Code は**空きメモリが減ると背面のコマンドを刈る**。時間では決まらず、20 秒で終わることも数時間走ることもある。
> <strong>12 時間を指定しても、そのぶん張り直すことになる。</strong>位置は保たれるので実害はない。**張り直しを前提にしておけばよい。**
...
> [!IMPORTANT]
> <strong>本番は `test-` で始まる ID と、`sandbox-` または `test-` で始まるルームを受け付けない。</strong>名乗る側・繋ぐ先として渡すと 400 で断られる。<strong>片付け（`archive` ・ `restore`）の対象としてなら渡せる。</strong>通さないと、過去に入った分を消せなくなるためである。**チャットへ投稿するテストが要るときは相談してほしい。**
